...
async function forward(request: Request, { params }: { params: Promise<{ path: string[] }> }) {
  ...
  if (request.method === "POST" && path.length === 2 && id && ["confirm-payment","cancel"].includes(path[1])) { const input = await request.json().catch(() => ({})); if (input && (typeof input !== "object" || Object.keys(input).some(key => key !== "cancellation_reason") || (input.cancellation_reason !== undefined && typeof input.cancellation_reason !== "string"))) return NextResponse.json({ detail: "Invalid booking request" }, { status: 422 }); backendPath = `${id}/${path[1]}`; body = JSON.stringify(path[1] === "cancel" ? input : {}); }
