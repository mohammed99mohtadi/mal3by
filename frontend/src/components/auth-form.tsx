...
export function AuthForm({ locale, mode, returnTo }: { locale: Locale; mode: "login" | "register"; returnTo?: string | null }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const body = mode === "login" ? { email: values.email, password: values.password, remember_me: values.remember_me === "on" } : { full_name: values.full_name.trim(), email: values.email, password: values.password, ...(values.phone_number.trim() ? { phone_number: values.phone_number.trim() } : {}) };
    try {
  ...
  return <form noValidate onSubmit={submit} className={mode === "login" ? "auth-form" : "mt-7 grid gap-5"}>
