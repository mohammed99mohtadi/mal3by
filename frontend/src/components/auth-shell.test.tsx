...
describe("AuthShell", () => {
  it("renders one English page heading and real reciprocal link", () => { render(<AuthShell locale="en" mode="login"><div>Form</div></AuthShell>); expect(screen.getAllByRole("heading", { level: 1 })).toHaveLength(1); expect(screen.getByRole("heading", { name: "Log in" })).toBeInTheDocument(); expect(screen.getByRole("link", { name: "Create account" })).toHaveAttribute("href", "/en/register"); });
