...
describe("AuthShell", () => {
  it("renders one English page heading, exact brand, and real reciprocal link", () => { render(<AuthShell locale="en" mode="login"><div>Form</div></AuthShell>); expect(screen.getAllByRole("heading", { level: 1 })).toHaveLength(1); expect(screen.getByRole("heading", { name: "Log in" })).toBeInTheDocument(); expect(screen.getByRole("link", { name: "MAL3ABY" })).toHaveTextContent("MAL3ABY"); expect(screen.getByRole("link", { name: "Create account" })).toHaveAttribute("href", "/en/register"); });
