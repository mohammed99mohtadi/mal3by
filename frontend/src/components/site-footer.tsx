...
export function SiteFooter({ locale, isLoggedIn }: { locale: Locale; isLoggedIn: boolean }) {
  ...
  return <footer aria-label={t.footerLabel} className="mt-auto border-t border-[var(--border-strong)] bg-[var(--surface-1)]">
    <div className="page-wrap grid gap-8 py-10 sm:grid-cols-[1fr_auto] sm:items-start">
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-3 text-sm font-semibold"><Link className="focus-ring hover:text-[var(--brand)]" href={`/${locale}/courts`}>{t.courts}</Link>{isLoggedIn && <Link className="focus-ring hover:text-[var(--brand)]" href={`/${locale}/bookings`}>{t.bookings}</Link>}<Link className="focus-ring hover:text-[var(--brand)]" href={`/${locale}/privacy`}>{locale === "ar" ? "الخصوصية" : "Privacy"}</Link><Link className="focus-ring hover:text-[var(--brand)]" href={`/${locale}/terms`}>{locale === "ar" ? "الشروط" : "Terms"}</Link><Link className="focus-ring hover:text-[var(--brand)]" href={`/${locale}/help`}>{locale === "ar" ? "المساعدة" : "Help"}</Link><LocaleSwitcher locale={locale} compact /></div>
